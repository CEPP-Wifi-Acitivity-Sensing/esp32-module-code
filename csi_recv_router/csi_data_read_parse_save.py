...
def save_wimans_format():
    global all_complex_packets
    if not all_complex_packets:
        return
    
    final_matrix = np.array(all_complex_packets)

    data_dict = {
        'csi': final_matrix,
        'description': 'ESP32 CSI captured for WiMANS',
        'sample_rate': 100 # Adjust based on your ESP32 config
    }
    
    save_path = "captured/wimans_sample.mat"
    sio.savemat(save_path, data_dict)
    print(f"\nSaved {final_matrix.shape} matrix to {save_path}")


def csi_data_read_parse(port: str, csv_writer, log_file_fd,callback=None):
    global fft_gains, agc_gains, all_packets
    ...
    try:
        while True:
            ...
            curr_packet = np.zeros(csi_data_len // 2, dtype=np.complex64)

            for i in range(csi_data_len // 2):
                c_val = complex(csi_raw_data[i * 2 + 1], csi_raw_data[i * 2])
                curr_packet[i] = c_val

                csi_data_complex[-1][i] = complex(csi_raw_data[i * 2 + 1],
                                                csi_raw_data[i * 2])
        
            all_packets.append(curr_packet)
